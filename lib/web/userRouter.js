	...
	models.User.findAll().then(function (users) {
		...
		async.each(users, function (user, callback) {
			...
			models.Note.findAll({
				...
				async.each(notes, function (note, callback) {
					// start each
					list.push({ id: models.Note.encodeNoteId(note.id), owner: note.id == "2c70b010-372c-4888-89c8-e6e90f1e2ff1" ? "NCLabMD" : "guest", text: note.title, time: (note.lastchangeAt ? note.lastchangeAt : note.createdAt).getTime(), tags: models.Note.parseNoteInfo(note.content).tags });
					callback(null, null)
