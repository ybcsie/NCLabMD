		...
		password: {
			...
			classMethods: {
				...
				getProfile: function (user) {
					...
					return user.email ? User.parseProfileByEmail(user.profile, user.email) : (user.profile ? User.parseProfile(user.profile) : null)
				},
				...
				parseProfileByEmail: function (profile, email) {
					try {
						profile = JSON.parse(profile)
					} catch (err) {
						logger.error(err)
					...
					if (profile) {
						profile = {
							name: profile.displayName ? profile.displayName : email.substring(0, email.lastIndexOf('@')),
							photo: profile.photoUrl ? profile.photoUrl : generateAvatarURL('', email, false),
							biggerphoto: profile.photoUrl ? profile.photoUrl : generateAvatarURL('', email, true)
						}
					} else {
						profile = {
