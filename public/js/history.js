...
export function parseMeall(list, callback) {
	checkIfAuth(
		() => {
			parseServerToMeall(list, callback)
		},
		() => {
		}
	)
...
export function parseLaball(list, callback) {
	checkIfAuth(
		() => {
			parseServerToLaball(list, callback)
		},
		() => {
			parseServerToLaball(list, callback)
		}
...
export function parseServerToMeall(list, callback) {
	$.get(`${serverurl}/meall`)
		.done(data => {
			if (data.meall) {
				parseToHistory(list, data.meall, callback)
			}
		})
		.fail((xhr, status, error) => {
			console.error(xhr.responseText)
...
export function parseServerToLaball(list, callback) {
	$.get(`${serverurl}/laball`)
		.done(data => {
			if (data.laball) {
				parseToHistory(list, data.laball, callback)
			}
		})
		.fail((xhr, status, error) => {
			console.error(xhr.responseText)
