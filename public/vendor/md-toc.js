...
(function (window) {
	...
	Toc.prototype._createTocContent = function recursiveToc(level = 0, titleElements = [], titleNames = [], ulClass = undefined, i = 0) {
		...
		for (var element; element = titleElements.shift();) {
			...
			if (elementTag == iterTag) {
				...
				content += recursiveToc(level + 1, titleElements, titleNames, ulClass, i)
			} else {
