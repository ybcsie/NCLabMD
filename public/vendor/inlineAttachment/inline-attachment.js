...
(function (document, window) {
	...
	inlineAttachment.defaults = {
		...
		progressTextImg: '![Uploading file...{filename}]()',
	...
	inlineAttachment.prototype.uploadFile = function (file, id) {
		...
		if (file.name) {
			...
			this.settings.extraParams.originFileName = file.name;
		}
	...
	inlineAttachment.prototype.onFileUploadResponse = function (xhr, id) {
		if (this.settings.onFileUploadResponse.call(this, xhr) !== false) {
			...
			if (result && filename) {
				var replacements = [];
				var string = (this.settings.extraParams.isImg ? this.settings.progressTextImg : this.settings.progressText).replace(this.filenameTag, id);
				var lines = this.editor.getValue().split('\n');
				var newValue = this.settings.extraParams.isImg ? this.settings.urlTextImg.replace(this.filenameTag, filename) : this.settings.urlText.replace(this.titleTag, this.settings.extraParams.originFileName).replace(this.filenameTag, filename);
	 ...
	 * @return {Void}
	 */
	inlineAttachment.prototype.onFileInserted = function (file, id, isImg) {
		if (this.settings.onFileReceived.call(this, file) !== false) {
			this.settings.extraParams.isImg = isImg;
			this.lastValue = (this.settings.extraParams.isImg ? this.settings.progressTextImg : this.settings.progressText).replace(this.filenameTag, id);
		...
		if (typeof clipboardData === "object") {
			...
			for (var i = 0; i < items.length; i++) {
				...
				if (this.isFileAllowed(item)) {
					...
					if (file !== null) {
						this.onFileInserted(file, id, true);
	 ...
	 * @return {Boolean} if the event was handled
	 */
	inlineAttachment.prototype.onDrop = function (e, imgOnly) {
		var result = false;
		for (var i = 0; i < e.dataTransfer.files.length; i++) {
			...
			else if (window.enableFileUpload && !imgOnly) {
				result = true;
				var id = ID();
				this.onFileInserted(file, id, false);
				this.uploadFile(file, id);
